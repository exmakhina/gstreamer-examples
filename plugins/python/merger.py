...
class Merger(Gst.Element):
	...
	def process_video(self, parent):
		...
		while True:
			...
			Gst.info("t_alloc %.3f  t_cp %.3f t_push %.3f" \
	...
	def setcaps_srcv(self, parent, caps):
		...
		Gst.info("other caps %s" % othercaps)

		if 1:
			outcaps = Gst.Caps('%s' % othercaps)

			out_s = outcaps[0]
			Gst.info("outcasp 0 %s" % (out_s))
			Gst.info(" %s" % dir(out_s))

			out_s.set_value("framerate", Gst.Fraction(30, 1))#caps[0]["framerate"])

			Gst.info("out_s %s" % out_s.to_string())

			Gst.info(" dir %s" % dir(outcaps))
			#outcaps.set_structure(0, out_s)
			outcaps = Gst.Caps.from_string(out_s.to_string())


			Gst.info("pad is %s" % self.srcvpad.__class__)
			Gst.info("pad can %s" % dir(self.srcvpad))
	...
	def _srcv_event(self, pad, parent, event):
		...
		Gst.info("event type %s" % event.type)
